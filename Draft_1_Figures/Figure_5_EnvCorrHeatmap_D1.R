...
# drop the pseudocount here
phy_pseudo <- transform_sample_counts(phy_filtered, function(OTU) OTU )
...
# move the pseudocount to here
combined_phylum <- transform_sample_counts(combined_phylum, function(OTU) OTU + 1)
...
corr_df_f[corr_df_f == "Root wash"] <- "Rhizosphere"

corr_df_f$Treatment <- as.factor(corr_df_f$Treatment)

ord <- c("Leaf","Root","Rhizosphere","Soil")

gf <- corr_df_f %>% mutate(Treatment =  factor(Treatment, levels = ord)) %>%
  arrange(Treatment) %>% 
  ggplot(aes(x = Y, y = X, fill = rho)) + 
  geom_tile(color="black") + facet_wrap(~Treatment, nrow = 4, scales = "free_y") +
  ...
  geom_hline(yintercept=seq(.5, length(unique(corr_df_f$X)),1), colour='grey')
...
corr_df_b <- bact_adj
corr_df_b[corr_df_b == "Root wash"] <- "Rhizosphere"

corr_df_b$Treatment <- as.factor(corr_df_b$Treatment)

ord <- c("Leaf","Root","Rhizosphere","Soil")

gb <- corr_df_b %>% mutate(Treatment =  factor(Treatment, levels = ord)) %>%
  arrange(Treatment) %>% 
  ggplot(aes(x = Y, y = X, fill = rho)) + 
  geom_tile(color="black") + facet_wrap(~Treatment, nrow = 4, scales = "free_y") +
  scale_fill_gradient2(low = "red", mid = "white",high = "blue") + theme_bw()  +
  ...
  theme(plot.title = element_text(size = 11, face = "bold")) + scale_y_discrete(limits=rev) + 
  theme(strip.text.x = element_text(size = 12,face = "bold")) +
  geom_hline(yintercept=seq(.5, length(unique(corr_df_b$X)),1), colour='grey')
...
ggsave("Fig5_Heatmap_D1.png", plot = the_heatmap, path = "Results_Figs_Tables/Figures_Draft1", dpi = 700, 
...
phy_pseudo
combined_asv <- transform_sample_counts(phy_pseudo, function(OTU) OTU + 1)
...
sample_data(combined_asv) <- sample_data(combined_asv)[, -which(names(sample_data(combined_asv)) %in%
                                                                        c("Soil.EC","UV.Light","Co2.ppm"))]
clean_asv <- combined_asv
Bacteria_asv <- subset_samples(clean_asv, Microbe == "bacteria")
Fungi_asv <- subset_samples(clean_asv, Microbe == "fung")
...
# Bacteria
bact_corr_heat <- variable_correlation(Bacteria_asv, 
                                       variables = c("Precipitation",
                                                     "Temperature",
                                                     "Humidity",
                                                     "Cloud.Cover",
...
##################################################################################################
# Investigate cloud cover

num_cols <- unlist(lapply(metadata, is.numeric), use.names = FALSE)

meatacopy <- metadata

colnames(meatacopy)[colnames(meatacopy) == "precip"] = "Precipitation"
colnames(meatacopy)[colnames(meatacopy) == "temp"] = "Temperature"
colnames(meatacopy)[colnames(meatacopy) == "humidity"] = "Humidity"
colnames(meatacopy)[colnames(meatacopy) == "cloudcover"] = "Cloud.Cover"
colnames(meatacopy)[colnames(meatacopy) == "Soil.pH"] = "Soil.pH"
colnames(meatacopy)[colnames(meatacopy) == "Soil.Moisture"] = "Soil.Moisture"
colnames(meatacopy)[colnames(meatacopy) == "Soil.Temp"] = "Soil Temperature"
colnames(meatacopy)[colnames(meatacopy) == "Organic.Matter"] = "Organic.Matter"
colnames(meatacopy)[colnames(meatacopy) == "Nitrate.N.ppm.N"] = "Nitrate.ppm"
colnames(meatacopy)[colnames(meatacopy) == "Potassium.ppm.K"] = "Potassium.ppm"
colnames(meatacopy)[colnames(meatacopy) == "Sulfate.S.ppm.S"] = "Sulfate.ppm"
colnames(meatacopy)[colnames(meatacopy) == "Calcium.ppm.Ca"] = "Calcium.ppm"
colnames(meatacopy)[colnames(meatacopy) == "Magnesium.ppm.Mg"] = "Magnesium.ppm"
colnames(meatacopy)[colnames(meatacopy) == "Sodium.ppm.Na"] = "Sodium.ppm"
...
cor_meta <- meatacopy[, colnames(meatacopy) %in% variables]

cor_meta <- na.omit(cor_meta)

m <- cor(cor_meta)
m

## plot cloud cover and location

leaf_df <- subset_samples(phy_combined, tissue == "Leaf")

cloud_df <- subset_taxa(leaf_df, Phylum == "Proteobacteria")
plot_bar(cloud_df, x="Location")
